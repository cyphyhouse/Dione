...
function i_min(ns: seq<NodeState>): int
    requires 0 < |ns|
    ensures 0 <= i_min(ns) < |ns|
    ensures forall i | 0 <= i < |ns| ::
                ns[i_min(ns)].arb <= ns[i].arb
{
    if |ns| == 1 then 0
    else
        var hi := |ns|-1;
        var i := i_min(ns[..hi]);
        if ns[i].arb <= ns[hi].arb then i else hi
}

function bv_extract(vec: bv11, pos: BitPos) : Bit
{
    if pos < 0 then
        0
    else
...
function applyMapSeq<U,V> (f: map<U,V>, xs:seq<U> ): seq<V>
    requires (set x | x in xs) <= domain(f)
    ensures  |xs| == |applyMapSeq(f, xs)|
    ensures  forall i | 0 <= i < |xs| :: f[xs[i]] == applyMapSeq(f, xs)[i]
{
    if xs == [] then [] else [f[xs[0]]] + applyMapSeq(f,xs[1..])
}
}
...
function eff'5_recv(s: State, act: Action): State
  ...
  ensures |eff'5_recv(s, act).nodes| == |s.nodes|
  ensures rel_eff'5_recv(s, act, eff'5_recv(s, act))
{
...
predicate rel_eff'5_recv(s: State, act: Action, s': State)
    requires pre'5_recv(s, act)
    ensures rel_eff'5_recv(s, act, s')
        ==> (|s'.nodes| == |s.nodes|
          && forall i | 0<= i < |s.nodes| :: s.nodes[i].arb == s'.nodes[i].arb)
{
    if s.pos >= 0 then
        |s'.nodes| == |s.nodes|
     && forall i | 0 <= i < |s.nodes| ::
         s'.nodes[i] ==(if s.nodes[i].transmit
                        && act.msg != bv_extract(s.nodes[i].arb, s.pos)
...
(pre'5_recv(s, act) && rel_eff'5_recv(s, act, s'))
}
